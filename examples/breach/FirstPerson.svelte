<script lang="ts">
	...
	export interface Scene {
		...
		word: string;
		hue: string;
		/** The card's headline number and its label, for a shot that states what
		 *  the board is about to do. See `Scene` in `cinema.ts`. */
		power: number;
		powerLabel: string;
		/** Who they are, as the select screen knows them — the geometry and the
		 *  plate colour of the body you are about to be standing in. The CARD's hue
		 *  above dresses the shot; this dresses the person, and they are different
		 *  facts: a Threat Hunter playing a purple card is still green. */
		skin: CharacterSkin;
		/** Override the shared suit grey — see `Scene` in `cinema.ts`. */
		suit?: string;
		/** Who this shot might name. Only `unmask` reads it. */
		lineup?: Lineup;
		/** Which staging to run. `insert` flies and dives; `blackout` walks. */
		shot: ShotKind;
	}
	...
	interface Props {
		...
		// ── The blackout shot ────────────────────────────────────────────────
		// Its own durations rather than reusing the insert's, because they are
		// not the same gesture at different speeds: `establishMs` is an arrival
		// and `spotMs` is a glance, and tuning one by way of the other means
		// every future adjustment to the loud shot quietly retimes the quiet one.

		/** The low look at the operator before you are them. Shorter than the
		 *  insert's establish — this shot has more to show later and cannot
		 *  afford to spend the same budget on a portrait. */
		spotMs?: number;
		/** Closing the distance on foot. The longest beat in the shot, and it has
		 *  to be: the whole claim is that this took patience. */
		creepMs?: number;
		/** A look along the wall before going in. */
		flankMs?: number;
		/** Hands up to the line. */
		reachMs?: number;
		/** The cut. Shorter than the eye can track, on purpose. */
		snapMs?: number;
		/** How near the low look gets, in globe radii. Nearer than `meetDistance`
		 *  and much less lifted — you are crouched beside them, not above them. */
		spotDistance?: number;
		spotTilt?: number;

		// ── The unmask shot ──────────────────────────────────────────────────
		/** Already watching. Short: this beat establishes nothing except that you
		 *  were here before the card was played. */
		watchMs?: number;
		/** The shortlist collapsing. The long one — it is the only beat in the
		 *  shot where anything is happening. */
		siftMs?: number;
		/** The name landing. */
		nameMs?: number;
		/**
		 * How far off the target this shot stands, in globe radii — and the one
		 * number that makes it a different shot rather than a differently dressed
		 * one.
		 *
		 * It never descends. `insert` and `blackout` both end at
		 * `SURFACE_DISTANCE`; this holds well above it for every frame, so the
		 * building never towers and you are never among it. Attribution's
		 * approach already happened, off screen, before the card was played.
		 */
	...
	/**
	 * Both shots' beats in one union, and `hold` / `rise` shared between them.
	 *
	 ...
	 * and a second `hold` would need a second `sustain`, a second `leave`, and a
	 * second chance to strand the HUD under a scrim.
	 */
	type Phase =
		| 'idle'
		| 'establish'
	...
	function anchorOf(id: string | null | undefined): { x: number; y: number; r: number } | null {
		...
		// Visual px in, layout px out — same conversion BoardFx makes, and for the
		// same reason: under the front page's `zoom` the reticle and the streaks
		// aimed short of the building they were locked onto.
		const z = cssZoom(host);
		return {
			x: (r.left + r.width / 2 - h.left) / z,
			y: (r.top + r.height / 2 - h.top) / z,
			r: Math.max(10, Math.max(r.width, r.height) / 2 / z)
		};
	}
	...
	function sustain() {
		...
		const step = (now: number) => {
			...
			elapsed = now - t0;
			// Two ways of standing still. The insert shot is a person at rest; the
			// blackout shot is a person who has just cut something and is listening
			// for whether it was heard, which is a different stillness and not a
			// quieter one.
			mesh.setCameraPose(
				s.shot === 'blackout'
					? stilledPose(aimPose, elapsed)
					: s.shot === 'unmask'
						? // Still on the glass, and still the least steady of the three.
							// A long lens does not stop shaking because the operator has
							// finished thinking.
							lensDrift(aimPose, elapsed, 0.34)
						: breathe(aimPose, elapsed)
			);
			readMarks(s);
	...
	/**
	 * Stage the shot. Resolves with the eye on the target and the window held
	 * open — the caller is then free to run whatever it likes underneath and call
	 * `hold` or `leave` when it is done.
	 *
	 * The dispatch is here and nowhere else. Both stagings end in the same state
	 * (`phase = 'hold'`, camera seized, `aimPose` set, `sustain()` running), which
	 * is what lets `hold`, `leave` and `cut` stay single implementations that do
	 * not know which shot they are ending.
	 */
	...
	/** Establish, dive, turn — the loud one. */
	async function enterInsert(s: Scene): Promise<void> {
	...
	/**
	 * ── The blackout staging ────────────────────────────────────────────────
	 *
	 * Everything the insert shot does loudly, this does on foot. The structural
	 * difference is one line and it is the whole shot: there is NO DIVE. The
	 * insert flies to the operator at `meetDistance` and is then fired down to
	 * the surface as a separate, violent beat. Here the descent is folded into
	 * the walk — `cameraLerp(meet → aim)` carries `viewDistance` from 1.34 to the
	 * surface across the creep, so arriving is something the body did rather than
	 * something that was done to the camera.
	 *
	 * That is also why the gait has to be real. A lerp between two poses with no
	 * gait on top is a dolly, and a dolly reads as a drone shot however slowly it
	 * moves. `creepPose` puts the weight on alternating feet and — the part that
	 * actually sells it — stops dead between steps.
	 */
	async function enterBlackout(s: Scene): Promise<void> {
		if (!mesh || !camera) return;
		const on = standId(s);
		// Low and close rather than high and wide: this operator is crouched, and
		// a heroic three-quarter lift on somebody trying not to be seen is the
		// wrong sentence in the first two frames.
		...
		// ── Spot ─────────────────────────────────────────────────────────────
		const from0 = entryPose;
		const meetR = () => Math.min(wide, tightFor(on) * 1.5);
		phase = 'spot';
		await drive(spotMs, (t) => {
			p = t;
			mesh!.setCameraPose(cameraApproach(from0, meet, t));
			readMarks(s);
			frameOn(on, wide + (meetR() - wide) * ease(t));
		...
		// ── Creep ────────────────────────────────────────────────────────────
		// The framing centre walks between the two plots, the same way the
		// insert's `turn` does — aimed at the target from the first frame it
		// would be somewhere off the edge of the world and the viewport would
		// lurch to find it.
		const midR = meetR();
		phase = 'creep';
		await drive(creepMs, (t) => {
			p = t;
			const k = ease(t);
			mesh!.setCameraPose(creepPose(cameraLerp(meet, aim, k), elapsed));
			readMarks(s);
			const a = mesh!.nodeScreen(on);
			const b = mesh!.nodeScreen(s.structureId);
			if (a && b) {
				frameOnPoint(
					a.x + (b.x - a.x) * k,
					a.y + (b.y - a.y) * k,
					midR + (tightFor(s.structureId) - midR) * k
				);
			}
		});
		if (phase !== 'creep') return;

		// ── Flank ────────────────────────────────────────────────────────────
		// A look along the wall. The gait keeps running underneath, because
		// checking a corner is not a thing you stop walking to do — and the beat
		// exists mostly so the reach that follows has something to be a change
		// FROM. An approach that runs straight into the work has no moment in it
		// where the player understands that a decision was taken.
		const creepEnd = elapsed;
		phase = 'flank';
		await drive(flankMs, (t) => {
			p = t;
			mesh!.setCameraPose(creepPose(peekPose(aim, t, 0.062), creepEnd + elapsed));
			readMarks(s);
			frameOn(s.structureId, tightFor(s.structureId));
		});
		if (phase !== 'flank') return;

		// ── Reach ────────────────────────────────────────────────────────────
		// The gait decays to nothing across this beat rather than being switched
		// off at its start. A body that stops moving between one frame and the
		// next has been paused; one that settles has arrived.
		const flankEnd = creepEnd + elapsed;
		phase = 'reach';
		await drive(reachMs, (t) => {
			p = t;
			const k = 1 - ease(t);
			mesh!.setCameraPose(
				creepPose(aim, flankEnd + elapsed, {
					bob: CREEP.bob * k,
					sway: CREEP.sway * k,
					lift: CREEP.lift * k,
					step: CREEP.step
			...
			readMarks(s);
			// A small push in as the hands come up. Not a zoom onto the building —
			// onto the WORK, which is nearer than the building is.
			frameOn(s.structureId, tightFor(s.structureId) * (1 - 0.12 * ease(t)));
		});
		if (phase !== 'reach') return;

		// ── Snap ─────────────────────────────────────────────────────────────
		phase = 'snap';
		await drive(snapMs, (t) => {
			p = t;
			mesh!.setCameraPose(snapPose(aim, t, 0.03));
			readMarks(s);
			frameOn(s.structureId, tightFor(s.structureId) * 0.88);
		});
	 ...
	 * ── The unmask staging ──────────────────────────────────────────────────
	 *
	 * The shortest `enter` of the three, and the one with the least in it, which
	 * is the design rather than an omission. There is no establish, no dive, no
	 * gait, and — the load-bearing part — NO DESCENT. `watchDistance` is held for
	 * every frame; the camera never gets near `SURFACE_DISTANCE`, so the target
	 * never towers and you are never standing among it.
	 *
	 * That is what makes it a different shot rather than a differently dressed
	 * one. Both other stagings spend their whole first half getting somewhere,
	 * because both are about arriving. Attribution's arrival already happened:
	 * the card requires a REVEALED foothold, so by the time it is legal to play
	 * you have been watching that building for some time. A shot that flew in
	 * would be dramatising the one part of this the player did not do.
	 *
	 * What moves instead is the frame — it closes on the target across the sift,
	 * which is a lens being worked, not a body walking.
	 */
	async function enterUnmask(s: Scene): Promise<void> {
		if (!mesh || !camera) return;
		const watch = mesh.poseFacing(s.structureId, {
			tilt: watchTilt,
		...
		// ── Watch ────────────────────────────────────────────────────────────
		// Settle onto what is already being looked at. `cameraApproach` rather
		// than a lerp for the same reason the insert's establish uses it — turn
		// to it, then come to rest on it — but ending far out rather than near.
		const from0 = entryPose;
		const wideR = () => Math.min(wide, tightFor(s.structureId) * 3.4);
		phase = 'watch';
		await drive(watchMs, (t) => {
			p = t;
			mesh!.setCameraPose(lensDrift(cameraApproach(from0, watch, t), elapsed, t));
			readMarks(s);
			frameOn(s.structureId, wide + (wideR() - wide) * ease(t));
		});
		if (phase !== 'watch') return;

		// ── Sift ─────────────────────────────────────────────────────────────
		// The pose does not change at all — only the drift, and the frame
		// closing. A camera that crept forward here would be answering a
		// question the shot is not asking.
		const watchEnd = elapsed;
		const openR = wideR();
		phase = 'sift';
		await drive(siftMs, (t) => {
			p = t;
			mesh!.setCameraPose(lensDrift(watch, watchEnd + elapsed));
			readMarks(s);
			frameOn(s.structureId, openR + (tightFor(s.structureId) * 1.6 - openR) * ease(t));
		});
		if (phase !== 'sift') return;

		// ── Name ─────────────────────────────────────────────────────────────
		// The drift falls away as the name lands. A long lens that steadies is
		// the cheapest way to say the operator has stopped searching — the shake
		// was never the camera, it was the looking.
		const siftEnd = watchEnd + elapsed;
		phase = 'name';
		await drive(nameMs, (t) => {
			p = t;
			mesh!.setCameraPose(lensDrift(watch, siftEnd + elapsed, 1 - ease(t) * 0.72));
			readMarks(s);
			frameOn(s.structureId, tightFor(s.structureId) * 1.6);
		});
	...
	/** Which shot is running. Read off the scene rather than off the phase: the
	...
	/** What the shot currently IS, top-left. The same overlay is used for watching
		...
		phase === 'walk'
			...
			: phase === 'sift'
				...
				: phase === 'name'
					...
					: unmask
						? 'named'
						: phase === 'establish' || phase === 'spot'
							? 'operator'
			: phase === 'dive'
				? 'taking control'
				: phase === 'creep' || phase === 'flank'
					? 'approach'
					: phase === 'reach' || phase === 'snap'
						? 'contact'
						: blackout
							? 'dark'
							: 'pov'
	...
	const maskPhase = $derived(
		phase === 'hold'
			? ('public' as const)
			: (phase as 'watch' | 'sift' | 'name' | 'rise')
	);

	const darkPhase = $derived(
		phase === 'hold'
			? ('dark' as const)
			: (phase as 'spot' | 'creep' | 'flank' | 'reach' | 'snap' | 'rise')
	);
	...
	const focus = $derived(
		phase === 'establish' || phase === 'dive' || phase === 'spot' ? (opMark ?? mark) : mark
	);

	/** The unmask never stands anybody on the board — you are the one watching,
	 *  and the person who matters is not here. So no operator marker at all. */
	const opShown = $derived(!unmask);
	...
	const opIn = $derived(
		phase === 'establish' || phase === 'spot'
			...
			: phase === 'dive'
				? Math.max(0, 1 - p / 0.34)
				: // The blackout shot has no dive to hide the handover, so the marker
					// simply walks off over the first third of the creep — by which time
					// the eye has something else to be looking at.
					phase === 'creep'
					? Math.max(0, 1 - p / 0.3)
					: 0
	);
	...
	const build = $derived(UNIT_BUILD[scene?.skin.shape ?? 'runner']);

	/** Built rather than inlined, and the ternary is load-bearing: `figureFacets`
	 *  spreads its options over the defaults, so passing `{ suit: undefined }`
	 *  would overwrite the default grey with nothing rather than fall back to it. */
	const figureOpts = $derived(scene?.suit ? { suit: scene.suit } : {});
	...
	{#if live && scene}
		...
		>
			...
			{#if opIn > 0 && opMark && opShown}
				...
				<g opacity={opIn}>
					...
					<ellipse
						...
						fill="rgba(0,0,0,0.5)"
					/>
					<!-- The operator, in the roster's own three materials — dark suit,
					     plate in the class colour, lit visor. Painted from `figureFacets`
					     rather than from a local model so this figure and the tile the
					     player picked them off are the same character: one geometry, one
					     palette, two cameras. -->
					<g transform="translate({stand.x} {stand.y})">
						{#each figureFacets(scene.skin, cf, figureOpts) as f, i (i)}
							<path d={f.d} fill={f.fill} stroke={f.edge} stroke-width="0.7" stroke-linejoin="round" />
						{/each}
					</g>
			...
			{#if bodyIn > 0 && bodyIn < 1}
				...
				>
					{#each figureFacets(scene.skin, atStep(passFrame, k), figureOpts) as f, i (i)}
						<path d={f.d} fill={f.glow ? '#FFFFFF' : hue} />
					{/each}
				</g>
			{/if}
		...
		<!-- Everything the quiet shot draws that the loud one does not. Mounted
		     after the shared SVG so the conduit and the hands sit over the operator
		...
		<!-- ── Visor ───────────────────────────────────────────────────────────
		     ...
		     Not worn by the unmask shot. Corner brackets are helmet furniture, and
		     a helmet says somebody came here; that shot's whole claim is that they
		     did not, and it brings its own barrel vignette instead. -->
		{#if !unmask}
		<div
