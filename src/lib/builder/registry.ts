// Every backdrop the app can paint — the five standalone families plus the
// Möbius compositions. Imported rather than restated so the builder's menu
// cannot drift from the ids that actually resolve.
import { BACKDROP_IDS, BLEND_MODES, DEFAULT_BLEND } from '../backdrop/backdrops.js';
import { ALL_PIECES } from '../mesh-studio/pieces/piece-catalogue.js';

/** Every building the piece catalogue can draw, for `PieceCrest`'s menu. Derived
 *  for the same reason `BACKDROP_IDS` is imported above: a hand-typed copy is a
 *  menu that silently stops offering the newest building. */
const PIECE_KEYS: readonly string[] = Object.keys(ALL_PIECES).sort();
...
export interface PropDef {
	...
	/**
	 * Let an `enum` hold SEVERAL of its options at once, stored comma-joined.
	 *
	 * A flag rather than a `'multi-enum'` kind because nothing downstream needs
	 * to care: the value is still a string, still drawn by the chips branch,
	 * still animatable as a `step` channel, and a single-valued default still
	 * satisfies "default is one of options". Only the chip's click handler and
	 * its pressed test change.
	 *
	 * `chips` is the only presentation that honours it — a native `<select>` and
	 * a swatch grid have no multi-select affordance, and inventing one would be
	 * a worse control than the chips already are.
	 */
	multi?: boolean;
...
/**
 * Shape fills and strokes: real colour VALUES, not token names, so the swatch
 * grid paints what it sets. The tokens would re-theme, but a wireframe rectangle
 * is scaffolding rather than product surface — it is meant to stay grey while
 * the components around it change, and `none` has to be in the same list as the
 * colours for a fill you can turn off.
 */
const SHAPE_COLORS = [
	'none',
	'#5FEAD5',
...
/** Stroke controls shared by every shape — one definition, three entries. */
const SHAPE_STROKE_PROPS: Record<string, PropDef> = {
	stroke: {
		kind: 'enum',
		label: 'Stroke',
		default: '#94A3B8',
	...
	strokeWidth: { kind: 'number', label: 'Stroke width', default: 1, min: 0, max: 12, step: 1 },
	dash: {
		kind: 'enum',
		label: 'Dash',
		default: 'solid',
	...
	opacity: { kind: 'number', label: 'Opacity', default: 1, min: 0, max: 1, step: 0.05 }
};

export const REGISTRY: ComponentMeta[] = [
	// ── Primitives ──────────────────────────────────────────────────────────────
	{
		// The one entry backed by NO library component: freeform text, for the
		// labels, section headings and callouts a layout needs before any of it
		// is real. Everything else in here carries semantic meaning, so people
		// were reaching for SectionBar to write a note.
		id: 'Text',
		label: 'Text',
		category: 'Primitives',
		defaultW: 200,
		...
		placeable: true,
		props: {
			content: { kind: 'textarea', label: 'Content', default: 'Label' },
			size: {
				kind: 'enum',
				label: 'Size',
				default: 'sm',
			...
			color: {
				...
				label: 'Color',
				default: 'fg-muted',
				options: ['fg', 'fg-muted', 'fg-dim', 'accent', 'warn', 'error', 'success']
			},
			align: {
				kind: 'enum',
				label: 'Align',
			...
			mono: { kind: 'boolean', label: 'Monospace', default: false },
			uppercase: { kind: 'boolean', label: 'Uppercase', default: false }
		}
	},
	{
		// Usually arrives by dropping an image file on the canvas, which fills in
		// `src` and the natural aspect. Still placeable from the palette, because
		// `placeable: false` means something specific here — an overlay opened by
		// a trigger, never a canvas item — and an Image is a canvas item that
		// simply starts empty. Dragging one in and pasting a URL is a real way to
		// use it; the card renders the same "no source" placeholder it would on
		// the canvas, so the palette is not promising anything it won't deliver.
		id: 'Image',
		label: 'Image',
		category: 'Display',
		defaultW: 320,
		...
		placeable: true,
		props: {
			src: { kind: 'textarea', label: 'Source (URL or data URI)', default: '' },
			...
			radius: { kind: 'number', label: 'Corner radius', default: 2, min: 0, max: 64, step: 1 },
			opacity: { kind: 'number', label: 'Opacity', default: 1, min: 0, max: 1, step: 0.05 }
		}
	},
	{
		// Wireframe scaffolding: a box standing in for something not designed yet,
		// a panel behind a group, a redaction over a screenshot. Like Text, backed
		// by no library component — a Card would carry meaning this must not.
		id: 'Rectangle',
		label: 'Rectangle',
		category: 'Primitives',
		defaultW: 200,
	...
	{
		// A rule, not a connector: it sits where it is put and joins nothing, which
		// is what a section divider or a crop mark needs. Anything that should
		// follow two components as they move is a connector instead.
		id: 'Line',
		label: 'Line',
		category: 'Primitives',
		defaultW: 200,
	...
	// ── Display ─────────────────────────────────────────────────────────────────
	{
		 ...
		 *
		 * Its real input is a `StripSpec[]` — an array of records with ~25 fields
		 * each — which no `PropDef` kind can express, and which does not justify
		 * adding one for a single consumer. So the builder chooses a named
		 * COMPOSITION and the four things worth varying per placement; per-strip
		 * authoring lives in the background studio, and this consumes its output
		 * by name.
		 *
		 * `mr robot` is the default deliberately: the palette mounts every
		 * placeable entry live, so the default is a per-frame cost in the toolbox
		 * as well as on the canvas. It is the cheapest preset — 3 strips, 1
		 * traveller, no hue flow, no ghost pass. `spider-verse` is roughly 10×
		 * that (every strip drawn twice for its misregistration ghost), so think
		 * before making it anyone's default.
		 */
		id: 'HorizonBackdrop',
		label: 'Backdrop',
		category: 'Display',
		...
		props: {
			preset: {
				...
				label: 'Backdrop',
				default: 'ash-drift',
				// From the module that owns the list, so the menu cannot drift from
				// what actually resolves.
				options: BACKDROP_IDS,
				presentation: 'chips',
				// Backdrops are LAYERS, and the looks worth having are combinations —
				// Ash Drift's settling dust under Current Field's ink. The value stays
				// a string (`parseStack` reads a comma-joined list), so a single id
				// keeps working and this needed no new prop kind.
				multi: true,
				animate: 'step',
				section: 'BACKDROP'
			},
			/**
			 * How each stacked layer composites over the ones below it.
			 *
			 * Inert on a single backdrop, and shown anyway rather than hidden behind
			 * a `showWhen` — that can only test one prop against a fixed set of
			 * values, and the question here is "does `preset` hold more than one
			 * id", which it cannot ask.
			 */
			blend: {
				kind: 'enum',
				label: 'Layer blend',
			...
			rainbow: { kind: 'boolean', label: 'Hue flow', default: false, animate: 'step' },
			rainbowSpeed: {
				kind: 'number',
				label: 'Hue cycle',
				default: 18,
				...
				max: 60,
				step: 1,
				showWhen: { key: 'rainbow', values: ['true'] }
			},
			strength: {
				kind: 'number',
				label: 'Opacity',
			...
			cell: { kind: 'number', label: 'Floor cell', default: 34, min: 12, max: 96, step: 2 },
			/**
			 * A composition authored in the Backdrop Studio, as JSON.
			 *
			 * `textarea` because there is no array kind — this is the one place the
			 * scalar-only PropDef vocabulary is worked around, and it is worked
			 * around by serialising rather than by adding a kind. Empty means "use
			 * `preset`"; the studio writes it on Apply. Not animatable: a cue that
			 * swapped an entire composition mid-beat would be a re-layout, not a
			 * transition.
			 */
			strips: { kind: 'textarea', label: 'Composition JSON', default: '', animate: 'none' },
			/**
			 * Per-family knobs from the studio, as `{ styles, params }` JSON.
			 *
			 * The same workaround `strips` uses, for the same reason: Ash Drift's
			 * grain scale and Long Scan's hatch angle are per-LAYER values, and a
			 * scalar prop cannot be keyed by which layer it belongs to. Empty means
			 * every family renders on its own defaults.
			 */
			layers: { kind: 'textarea', label: 'Layer knobs (JSON)', default: '', animate: 'none' }
		}
	},
	{
		id: 'StatStrip',
		...
		props: {
			items: {
				...
				label: 'Items (JSON)',
				default:
					'[{"value":"12","label":"Agents"},{"value":"3","label":"Blocked","color":"danger"},{"value":"1","label":"Observing","color":"warn"}]'
			}
		}
	},
	{
	...
	},
	{
		...
		props: {
			type: {
				...
				label: 'Chart type',
				default: 'line',
				options: ['line', 'grouped-bar', 'stacked-bar', 'scatter', 'pie', 'radar', 'heatmap', 'candlestick', 'gauge']
			}
		}
	},
	{
	...
	},
	{
		...
		props: {
			stackRows: {
				...
				label: 'Rows (JSON array)',
				default:
					'[{"glyph":"shield-alert","label":"No deletion schedule for PII event streams","body":"Answered \\"no\\"; the scale needs \\"yes\\".","trailing":"46d","accent":"#FB7185"},{"glyph":"alert-triangle","label":"Revocation SLA is 5 business days, not 24 hours","trailing":"52d"}]'
			}
		}
	},
	{
	...
	},
	{
		...
		props: {
			...
			value: { kind: 'text', label: 'Selected value', default: 'medium' },
			label: { kind: 'text', label: 'Accessible label', default: 'Impact' },
			wrap: { kind: 'boolean', label: 'Wrap at the ends', default: false }
		}
	},
	{
		id: 'HexShield',
	...
	},
	{
		...
		props: {
			...
			mesh: { kind: 'boolean', label: 'Mesh', default: true }
		}
	},
	{
		id: 'ArmornetCrestHub',
	...
	{
		id: 'ArmornetCrestMesh',
		label: 'Armornet Crest — Mesh',
		category: 'Display',
		defaultW: 120,
		defaultH: 120,
		resizable: false,
	...
	{
		// Sized by its own `size` prop, like the brand marks above — the art does
		// not follow the frame, so dragging the frame would leave it behind.
		id: 'PieceCrest',
		label: 'Piece Crest',
		category: 'Display',
		defaultW: 80,
		...
		placeable: true,
		props: {
			// Read off the catalogue, never restated. A building added in
			// `pieces-civic.ts` has to appear in this menu without anybody
			// remembering that this menu exists.
			piece: {
				kind: 'enum',
				label: 'Piece',
				default: 'house',
			...
			size: { kind: 'number', label: 'Size', default: 46, min: 16, max: 240, step: 2 },
			color: { kind: 'text', label: 'Colour', default: 'var(--accent)' },
			offline: { kind: 'boolean', label: 'Offline', default: false }
		}
	},
