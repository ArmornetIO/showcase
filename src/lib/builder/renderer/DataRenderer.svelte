<script lang="ts">
	...
	const stackRows = $derived(
		parseJson<StackPreviewRow[]>(props.stackRows, [
			{
				glyph: 'shield-alert',
				label: 'No deletion schedule for PII event streams',
				body: 'Answered "no"; the scale needs "yes".',
				trailing: '46d',
				accent: '#FB7185'
			},
			{
				glyph: 'alert-triangle',
				label: 'Revocation SLA is 5 business days, not 24 hours',
				body: 'Answered "partial"; the scale needs "yes".',
				trailing: '52d'
			},
			{
				glyph: 'alert-triangle',
				label: 'Sub-processor list published but changes not notified',
				trailing: '46d'
			}
		])
	);
...
{#if componentId === 'DataTable'}
	{#if e('variant', 'kv') === 'kv'}
		<!-- kv mode reads `kvRows`, not `rows` — passing the pairs as `rows` drew an
		     empty panel. -->
		<DataTable variant="kv" {kvRows} />
	{:else}
...
{:else if componentId === 'CollapsibleStack'}
	<!-- The real component is snippet-driven — each call site owns what its rows
	     carry — so the builder gets a data-shaped preview rather than the full
	     API. Enough to lay one out and see its density; the meta marks that make
	     a real row useful are the consumer's. -->
	<CollapsibleStack>
		{#each stackRows as r, i (i)}
			<CollapsibleStackRow
				key={String(i)}
				glyph={r.glyph as IconName | undefined}
				glyphColor={r.accent ?? 'var(--fg-dim)'}
				accent={r.accent}
				trailing={r.trailing}
			>
				{#snippet label()}{r.label}{/snippet}
				{#if r.body}<div class="text-[0.8rem] text-[var(--fg-muted)]">{r.body}</div>{/if}
			</CollapsibleStackRow>
		{/each}
	</CollapsibleStack>
