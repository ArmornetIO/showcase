<script lang="ts">
	...
	interface DevCogProps {
		...
		showPerf?: boolean;
		/**
		 * Host tools for the QA drawer. Receives the nits controller so a tool can
		 * borrow the one element inspector (`nits.pickOnce`) instead of shipping a
		 * second armed-click mode that fights it for the same clicks.
		 */
		qaContent?: Snippet<[NitsController]>;
