<script lang="ts">
	...
	import WasmProbe from './WasmProbe.svelte';
		...
		<QaSection label="nits{nits.count > 0 ? ` (${nits.count})` : ''}" grow>
			{#snippet actions()}
				{#if nits.count > 0}
					...
					{:else}
						<GhostButton onclick={() => nits.selectAll()} title="Select every nit">all</GhostButton>
					{/if}
					...
					<GhostButton
						...
						title={nits.selectedCount > 0
							? `Copy the ${nits.selectedCount} selected nit(s)`
							: 'Copy every nit'}
					>
						{#if nits.copied}
							✓ copied
