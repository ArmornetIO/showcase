...
<div class="w-full">
	...
	{#if stepStyle === 'blocks'}
		...
		<div class="flex gap-[3px] w-full">
			{#each stepArray as step, i}
				...
				>
					<span
						class="block w-full h-[6px] rounded-[2px] border border-[var(--border)] motion-reduce:transition-none!"
	...
	{:else if stepStyle === 'ticks'}
		<div
			class="relative w-full h-[6px] bg-[rgba(15,23,42,0.5)] border border-[var(--border)] rounded-[3px] overflow-hidden"
		>
			<div
		...
		{#if stepArray.some((s) => s.label && isNaN(Number(s.label)))}
			<div class="relative h-[16px] mt-[5px]">
				{#each stepArray as step, i}
					<span
						class="absolute -translate-x-1/2 font-[var(--mono)] text-[0.5rem] tracking-[0.08em] text-[var(--fg-dim)] uppercase whitespace-nowrap"
						style:left="{(i / (total - 1)) * 100}%">{step.label}</span
					>
				{/each}
			</div>
		{/if}
	...
	{:else}
		<div
			class="w-full h-[4px] bg-[rgba(15,23,42,0.5)] border border-[var(--border)] rounded-[2px] overflow-hidden mb-[8px]"
		>
			<div
				class="h-full motion-reduce:transition-none!"
				style:background={FILL_GRADIENT[variant ?? 'accent']}
