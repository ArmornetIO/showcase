<script lang="ts">
	...
	import type { BackdropChoice } from '../settings/store.svelte.js';
	...
	interface AppShellProps {
		...
		/**
		 * How strongly to paint the glass ground behind the page.
		 * - `'deep'` (default) — half-strength aurora and a soft, ramped grid.
		 *   The right choice under dense content, where a full-strength wash
		 *   competes with tables and forms.
		 * - `'bright'` — the full aurora and the crisp hairline grid. For pages
		 *   where the ground is part of the composition rather than a backdrop:
		 *   canvases, and low-density showpieces like first-run onboarding.
		 */
		ground?: 'deep' | 'bright';
		/**
		 * Add the horizon layer to the ground: two planes raked to a vanishing
		 * point at the centre line, with their cross-ties travelling toward the
		 * viewer. A flat grid reads as a surface; a converging one reads as
		 * distance, which is what makes the content feel like it sits in front
		 * of something rather than on it.
		 *
		 * Off by default. It is a backdrop, so it is deliberately something a
		 * page opts into rather than something every page inherits — and its
		 * strength is `--glass-horizon`, not a hardcoded alpha, so it can be
		 * dialled per theme or switched off entirely without touching markup.
		 */
		horizon?: boolean;
		/**
		 * A named backdrop composition, or `none`. Supersedes `horizon`, which
		 * stays for callers that just want the default ground and have no opinion
		 * about which one.
		 */
		backdrop?: BackdropChoice;
		/** 0–1, applied to the backdrop layer as `--backdrop-strength`. */
...
>
	...
	{/if}
	<main
		class="app-shell-main glass-stage"
		class:glass-stage-bright={ground === 'bright'}
		id="app-main-content"
		tabindex="-1"
	>
		<!--
			Behind the page, inside the stage's clip, and inert to the pointer.

			BEHIND is carried by `z-index: -1` on the wrapper, not by DOM order.
			The backdrop is absolutely positioned and the page content is not, and
			CSS paints positioned boxes ABOVE in-flow content regardless of which
			came first — so a backdrop with an opaque ground covered the page it
			was supposed to sit behind. `.glass-stage` is already `isolation:
			isolate`, so the negative index cannot escape past the stage's own
			background, and this is the same move `.glass-stage::before` makes for
			the aurora layer.

			`{#if}` rather than an opacity-0 layer: `none` has to be genuinely
			free. A backdrop left mounted at zero opacity still composites its
			nested filter layers every frame, which is the opposite of what
			someone choosing "None" is asking for.
		-->
		{#if backdrop !== 'none'}
			<div class="app-shell-backdrop">
				<Backdrop id={backdrop} strength={backdropStrength} />
			</div>
		{:else if horizon}
			<div class="app-shell-backdrop">
				<HorizonBackdrop />
...
<style>
	...
	/* The one job here is `z-index: -1` — see the comment at the call site. The
	   rest just gives the backdrop the stage's box to fill. */
	.app-shell-backdrop {
		position: absolute;
		inset: 0;
		z-index: -1;
	...
	.app-shell-main {
		...
		overflow-x: clip;
		min-height: 100vh;
		/* background-COLOR, not the `background` shorthand: `.glass-stage` sets
		   the grid via background-image, and this rule is unlayered component
		   CSS, so it outranks `@layer components` and the shorthand would reset
		   the image to none. The stage owns the image; this owns the colour. */
