		...
		<div class="flex flex-wrap gap-[0.875rem] items-center justify-center mt-2">
			...
			{:else}
				{#if primaryLabel && primaryHref}
					<a
						class="inline-flex items-center px-8 py-3 rounded-[3px] font-[var(--mono-display)] text-sm font-bold tracking-[0.1em] uppercase no-underline transition-[opacity,transform] duration-200 bg-[var(--accent)] text-[var(--bg)] hover:opacity-[0.88] hover:-translate-y-px"
						href={primaryHref}
					>{primaryLabel}</a>
				{/if}
				{#if secondaryLabel && secondaryHref}
					<a
						class="inline-flex items-center px-8 py-3 rounded-[3px] font-[var(--mono-display)] text-sm font-bold tracking-[0.1em] uppercase no-underline transition-[opacity,transform] duration-200 bg-transparent border border-[rgba(94,234,212,0.35)] text-[var(--accent)] hover:opacity-[0.88] hover:-translate-y-px"
						href={secondaryHref}
					>{secondaryLabel}</a>
				{/if}
			{/if}
