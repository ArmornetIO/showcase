<script lang="ts">
	...
	import EdgeParticles from './EdgeParticles.svelte';
	import type { ParticleRun } from './gl/particle-instances.js';
	...
	// The link geometry moved to primitives/canvas so `Edge` draws the same curve
	// this does — see edge-path.ts. `bowControl` is still needed here on its own
	// because `edgePointAt` evaluates the quadratic to pin labels to the line.
	import {
		bowControl,
		edgePathBetween,
		edgeNormal as normalBetween
	} from '../primitives/canvas/edge-path.js';
	...
	function edgeNormal(e: StudioEdge): { nx: number; ny: number } {
		...
		return normalBetween(ep.a, ep.b);
	}
	...
	function edgePath(e: StudioEdge): string {
		const ep = edgeEndpoints(e);
		if (!ep) return '';
		return edgePathBetween(ep.a, ep.b, edgeCurve);
	}

	/** A colourless edge — one carrying no data type, only a weight. White at
	 *  alpha was the old spelling and it vanishes on a pale ground; the ink has
	 *  to follow the theme's foreground, which is what `--fg` already is. */
	function neutralEdge(pct: number): string {
		return `color-mix(in srgb, var(--fg) ${pct}%, transparent)`;
	}
	...
	function edgeStroke(e: StudioEdge): string {
		if (e.style) {
			switch (e.style) {
				case 'energy':    return meshInk(tuning.energyColor);
				case 'pulse':     return meshInk('rgba(34,211,238,0.22)');
				case 'dashed':    return neutralEdge(35);
				case 'degraded':  return meshInk(tuning.degradedColor);
				case 'blocked':   return meshInk(tuning.offlineColor);
				case 'latent':    return neutralEdge(18);
				case 'scanning':  return meshInk('rgba(56,189,248,0.5)');
				case 'encrypted': return meshInk('rgba(110,231,183,0.5)');
			}
		}
		if (e.edgeState === 'degraded') return meshInk(tuning.degradedColor);
		if (e.edgeState === 'offline')  return meshInk(tuning.offlineColor);
		if (e.dataType) return meshInk(DATA_TYPE_COLOR[e.dataType as keyof typeof DATA_TYPE_COLOR] ?? 'var(--border-strong)');
	...
	function edgeChipColor(e: StudioEdge): string {
		if (e.style) {
			switch (e.style) {
				case 'pulse':     return meshInk('rgb(34,211,238)');
				case 'dashed':    return neutralEdge(90);
				case 'latent':    return neutralEdge(75);
				case 'scanning':  return meshInk('rgb(56,189,248)');
				case 'encrypted': return meshInk('rgb(110,231,183)');
			}
		}
		return edgeStroke(e);
	}
	...
	function particleColor(e: StudioEdge): string {
		return meshInk(e.dataType ? DATA_TYPE_COLOR[e.dataType as keyof typeof DATA_TYPE_COLOR] : '#22D3EE');
	}
	...
	function needsMotionPath(e: StudioEdge): boolean {
		return e.style === 'scanning' || e.style === 'pulse';
	}

	/** The energy streams, resolved for the GPU layer.
	 *
	 *  Only `isParticleEdge` runs move here. `scanning` and `pulse` are two and one
	 *  circle respectively and stay in SVG — they were never the cost, and porting
	 *  them would mean carrying their keyPoints reversal and their separate glow
	 *  into the shader for no measurable return. */
	function particleRuns(list: StudioEdge[]): ParticleRun[] {
		if (maxParticles <= 0) return [];
		const out: ParticleRun[] = [];
		for (const e of list) {
			if (!isParticleEdge(e)) continue;
			const ep = edgeEndpoints(e);
			if (!ep) continue;
			out.push({
				a: ep.a,
				b: ep.b,
				curve: edgeCurve,
				color: particleColor(e),
				dur: particleDur(e),
				count: Math.min(e.particleCount ?? maxParticles, maxParticles),
				size: tuning.particleSize
			});
		}
		return out;
	}

	// Split the same way the SVG layer is: by default all energy rides above the
	// nodes, and `depthSortedParticles` puts base-edge energy back underneath.
	const baseRuns = $derived(depthSortedParticles ? particleRuns(baseEdges) : []);
	const topRuns = $derived(particleRuns(depthSortedParticles ? raisedEdges : edges));
	...
	function sweepColor(n: StudioNode): string {
		return meshInk(n.state === 'degraded' ? 'rgba(250,204,21,0.35)' : n.state === 'offline' ? 'rgba(248,113,113,0.25)' : 'rgba(95,234,213,0.4)');
	}
	...
	function activePortColor(n: StudioNode): string {
		for (const p of nodePorts(n)) {
			if (n.portFlow?.[p.id]?.active) return meshInk(DATA_TYPE_COLOR[p.dataType]);
		}
		return stroke(n);
	}
	function traceColor(n: StudioNode): string {
		if (n.state === 'degraded') return meshInk(tuning.degradedColor);
		if (tuning.traceColorMode === 'fixed') return meshInk(tuning.traceColor);
	...
	// `stroke`/`fill` are the only two places a node's colour is decided, which is
	// why the theming goes here and not at the ~40 attributes that consume them.
	// Whatever the caller supplied is a hue, not a value — see palette.svelte.ts.
	function stroke(n: StudioNode) { return meshInk(n.strokeColor ?? MESH_NODE_COLOR[n.type]); }
	function fill(n: StudioNode) { return meshInk(n.fillColor ?? MESH_NODE_FILL[n.type]); }
	...
	/*  A caption is text before it is a mark, so its neutral is the theme's
	 *  foreground rather than a fixed pale slate — that literal was legible only
	 *  because every theme it had ever run in was dark. */
	const LABEL_INK = 'color-mix(in srgb, var(--fg) 82%, transparent)';
	function labelInk(n: StudioNode): string {
		if (!tuning.worldHue) return stroke(n);
		return n.state === 'degraded'
			? meshInk(tuning.degradedColor)
			: n.state === 'offline'
				? meshInk(tuning.offlineColor)
				: LABEL_INK;
	}
...
<!-- Base-edge energy, when depthSortedParticles asks for it behind the nodes.
     A canvas cannot be interleaved INTO the svg, so it sits under the whole
     layer rather than between the base edges and the nodes: energy is dimmed
     very slightly by the translucent edge stroke it runs along, where before it
     painted over it. That is the one visible cost of the port, it only applies
     in the depth-sorted mode, and the alternative is keeping SMIL. -->
{#if baseRuns.length}
	<EdgeParticles runs={baseRuns} />
{/if}
...
>
		<defs>
			...
			<marker id="ms-arrow-flow" markerWidth="7" markerHeight="7" refX="5" refY="3" orient="auto">
				<path d="M0,0 L0,6 L7,3 z" fill="var(--palette-cyan)" opacity="0.9" />
			</marker>
		...
		{#snippet edgeLayer(list: StudioEdge[])}
			...
			{#if flow}
				{#each list as e (e.id)}
					...
					{#if fd !== undefined}
						{@const d = edgePath(e)}
						{@const col = meshInk(flowDepthColor(fd))}
		...
		{#snippet particleLayer(list: StudioEdge[])}
			{#each list as e (e.id)}
				...
				{#if maxParticles > 0 && e.style === 'scanning'}
		...
		<g transform="translate({transform.tx},{transform.ty}) scale({transform.tk})">
			...
			{#each nodes as n (n.id)}
				...
				>
					...
					{#if isOrigin && !piece}
						<circle r={r + 22} fill="none" stroke="var(--palette-cyan)" stroke-width={1} opacity={0.2} class={showAnimate ? 'ms-source-outer' : ''} />
						<circle r={r + 10} fill="none" stroke="var(--palette-cyan)" stroke-width={1.5} opacity={0.5} class={showAnimate ? 'ms-source-pulse' : ''} />
					{/if}
					...
					{#if isOrigin}
						<text class="ms-t" y={-(r + 26)} font-size={8} font-weight={700} letter-spacing="0.1em" fill="var(--palette-cyan)" pointer-events="none">ORIGIN</text>
					{/if}
				</g>
			{/each}
...
<!-- Energy above the nodes — the default, so a link's traffic is never occluded
     by the things it connects. -->
{#if topRuns.length}
	<EdgeParticles runs={topRuns} />
{/if}
