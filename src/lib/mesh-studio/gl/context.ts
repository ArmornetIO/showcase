...
export function updateBuffer(
	...
	gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
	// Reads back stored object state — no pipeline sync, unlike a readPixels — so
	// it is cheaper than tracking capacity in a side table that a second context
	// could then read wrong.
	const capacity = gl.getBufferParameter(gl.ARRAY_BUFFER, gl.BUFFER_SIZE) as number;
	if (data.byteLength <= capacity) gl.bufferSubData(gl.ARRAY_BUFFER, 0, data);
