<script lang="ts">
	...
	/** The whole graticule as TWO path strings, near and far.
	 *
	 *  One `<path>` per run is the obvious shape and was the expensive one: the
	 *  run count changes as rings cross the horizon, so an `{#each}` over it
	 *  destroyed and rebuilt SVG children on every frame of a spin — and in dev
	 *  Svelte answers repeated writes to the same source within one flush by
	 *  capturing a stack trace per write (`Error.stackTraceLimit = Infinity`),
	 *  which is how a decorative grid came to own a third of the frame. Runs
	 *  differ only in near/far, so two subpath strings say the same thing with
	 *  two attribute writes and no reconciliation at all. */
	const web = $derived.by(() => {
		if (!(radius > 0)) return { front: '', back: '' };
		let front = '';
		let back = '';
		...
		for (const ring of grid) {
			for (const r of runs(ring.map((s) => ({ p: spin(s.p, yaw, pitch), lift: s.lift })))) {
				if (r.front) front += r.d;
				else back += r.d;
			}
		}
		return { front, back };
...
<svg class="gf" aria-hidden="true">
	...
	<g transform="translate({transform.tx},{transform.ty}) scale({transform.tk})">
		...
		<path d={web.back} fill="none" stroke={color} stroke-width={0.4} opacity={0.03} />
		<path d={web.front} fill="none" stroke={color} stroke-width={0.5} opacity={0.08} />
	</g>
</svg>
