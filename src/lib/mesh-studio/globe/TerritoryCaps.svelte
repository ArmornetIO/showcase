<script lang="ts">
	...
	//
	// The RAISED wall is drawn by `TerritoryWalls` on the GPU; this file still
	// solves its rim, because that is where the two rules below live. The FLAT
	// wall stays here as SVG — it is a screen-space parapet rather than 3D, and
	// it is a handful of paths that only exist when `wallHeight` is 0.
	//
	...
	import TerritoryWalls from './TerritoryWalls.svelte';
	import type { Pt, Seg, WallCapInput } from '../gl/wall-geometry.js';
	import type { WallPaint } from '../gl/wall-shaders.js';

	// A territory's colour arrives as whatever hex its scheme was authored in —
	// a dark-ground pastel. `hue` is where that becomes the current theme's step
	// for the same hue; see palette.svelte.ts. Every paint of `t.color`/`t.glow`
	// in this file goes through it, including the canvas pass, which re-runs on
	// a theme change because the resolver is reactive.
	//
	// Named `hue` rather than `ink` because the wall block below already binds a
	// local `ink` for "the colour this territory is drawn in right now", and a
	// shadowed helper there would be a silent no-op.
	const hue = (c: string) => meshInk(c);
		...
		for (let i = 0; i <= SAMPLES; i++) {
			...
			if (s.z >= 0.02) {
				...
				run.push({
					...
					// Carried, not recomputed downstream: with no depth buffer the GL
					// wall sorts its panes back to front, and this is the only thing that
					// knows how near the eye each sample is.
					z: s.z
				});
			} else if (run.length) {
				runs.push(run);
	 ...
	 *
	 *  Flat wall only. With no height the two edges are the same line, and the
	 *  raised wall's footing and crest are quads in `wall-geometry` — a stroked
	 *  path cannot be depth-sorted against the pane it sits on. */
	...
	// The tint, the names and the flat wall's parapet stay SVG, because they are a
	// handful of elements that benefit from being real nodes. The floor is the
	// opposite: thousands of segments that must be re-projected on every frame the
	// globe turns. The raised wall lost the same argument later and went to the
	// GPU — see `TerritoryWalls`.
	//
	...
	$effect(() => {
		...
		for (const s of shown) {
			...
			// Resolved once per territory, not once per pass: three strokes below
			// share it, and the resolver is cached but not free.
			const base = hue(s.t.color);
			const lifted = s.focused ? hue(s.t.glow ?? s.t.color) : base;
		...
		territories
			...
			.map((s) => {
				...
				return {
					...s,
					face: facing(s.c.z),
					// The rim itself, handed to the GL layer. Every path field below is
					// built ONLY when the wall is flat: `shown` re-derives on every yaw
					// change, so serialising ~72 samples four times per region was a
					// per-frame cost paid to hand the browser a `d` it had to re-parse,
					// re-tessellate and re-rasterise. That is what moving to the GPU
					// bought, and the strings are not built when nothing reads them.
					runs,
					base: raised ? '' : edgePath(runs, 'base'),
					crest: raised ? '' : edgePath(runs, 'top'),
	...
	/** What the GL layer draws. Colour precedence and the focus beat are resolved
	 *  here rather than there, so the wall and the floor beneath it cannot end up
	 *  disagreeing about which hue a territory is. Empty when the wall is flat —
	 *  that parapet is screen-space ornament and stays in the SVG below. */
	const walls = $derived(
		raised
			? shown.map(
					(s): WallCapInput & WallPaint => ({
						key: s.t.key,
						runs: s.runs,
						ink: hue(s.focused ? (s.t.glow ?? s.t.color) : s.t.color),
						lit: s.focused ? Math.min(1, progress * 2.2) : 0,
						face: s.face,
						glow: s.focused && !moving
					})
				)
			: []
	);
...
<!-- The raised wall. Between the floor and the SVG, so it stands ON the ground it
     encloses and still has the tint and the names painted over it. Always mounted,
     even when the wall is flat and it has nothing to draw: a GL context is a
     capped page resource, and tearing one down every time the height slider
     crosses zero is how a globe runs out of them. -->
<TerritoryWalls caps={walls} postEvery={POST_EVERY} />
...
<svg class="caps" aria-hidden="true">
	...
	<g transform="translate({transform.tx},{transform.ty}) scale({transform.tk})">
		...
		<!-- The FLAT wall — a parapet, drawn only when the wall has no height to
		     read and so has to say "wall" some other way. The raised wall left this
		     document entirely; it is quads on the GPU now, because a transparent
		     pane needs its neighbours sorted behind it and a stroked path cannot be.
		...
		{#each raised ? [] : shown as s (s.t.key)}
			{@const lit = s.focused ? Math.min(1, progress * 2.2) : 0}
			{@const ink = hue(s.focused ? (s.t.glow ?? s.t.color) : s.t.color)}
			{@const a = s.face}
			<!-- The bloom is dropped WHILE THE CAMERA MOVES.
			     A Gaussian blur over a whole territory has to be re-rasterized on any
			     frame where its contents or its scale change, and during a flight both
			     change every frame. Measured on the arrival flight, this alone was the
			     difference between a worst frame of ~34ms and ~26ms: it does not move
			     the median, it makes the spikes, which is exactly what a hand reads as
			     lag.
			...
			<g opacity={a} style:filter={s.focused && !moving ? 'url(#caps-glow)' : null}>
				...
				/>
				<!-- The crest last, so it stays the sharpest thing on the wall. With no
				     height, base and crest are the same line. -->
				<path
					d={s.crest}
					fill="none"
					stroke={ink}
			...
			{#each shown as s (s.t.key)}
				<g transform="translate({s.p.x + cx},{s.p.y + cy}) scale({inv})">
					<text
						...
						fill={hue(s.focused ? (s.t.glow ?? s.t.color) : s.t.color)}
