...
// Fan / Globe"), factored out so any mesh canvas can present the SAME options and
// place nodes the SAME way. The 2D layouts (grouped/packed/radial/fan) are solved
// here — seed a feasible arrangement around the hub, then separate. 'globe' is
// intentionally not solved here: it solves for DIRECTIONS on a sphere and projects
// them every frame, so it's the caller's job (it owns the spin state).
...
export type MeshLayoutId = 'grouped' | 'packed' | 'radial' | 'fan' | 'globe';
