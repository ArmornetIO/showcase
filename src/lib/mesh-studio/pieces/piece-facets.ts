...
export interface Facet {
	...
	depth: number;
	verts: PieceVert[];
	/** How edge-on this face is to the viewer: 0 square-on, 1 grazing.
	 *
	 *  The hologram term, and the one that separates projected light from painted
	 *  plastic. A lit solid is BRIGHTEST where it faces you, because that is where
	 *  it reflects most; a volume of glowing particles is brightest where you look
	 *  ALONG it, because that is where there is most of it between you and the
	 *  background. Driving opacity off this rather than off `shade` is most of the
	 *  difference between a chess piece and a hologram, and it costs one dot
	 *  product that the cull is already computing. */
	fresnel: number;
	/** False when the face points away and is only being kept because the caller
	 *  asked to see through the piece. */
	front: boolean;
}
...
export interface FacetOpts {
	...
	scale?: number;
	light?: Vec3;
	/** Keep the faces pointing away as well, flagged `front: false`.
	 *
	 *  Off by default, and it must stay off by default: for an opaque solid a back
	 *  face is not merely wasted, it is WRONG — the painter's sort would draw the
	 *  inside of the far wall over the near one. It exists for the transparent
	 *  case, where seeing a thing's own far side is not a bug but the whole point.
	 *  You cannot see through a chess piece. */
	showBack?: boolean;
}
...
export function pieceFacets(piece: Piece, frame: TangentFrame, opts: FacetOpts = {}): Facet[] {
	...
	for (const solid of piece) {
		for (const face of solid.faces) {
			...
			// Facing away — the piece's own front will cover it, unless the caller
			// wants to see through the piece.
			const front = nrm.z / len > 0.001;
			if (!front && !opts.showBack) continue;
			...
			out.push({
				...
				// |n·v| with v = +z, so a face square to the viewer is 0 and one seen
				// edge-on is 1. Not banded: the whole value of this term is the smooth
				// ramp across a curved run of facets, which is what reads as a
				// continuous surface of light rather than as a row of lit plates.
				fresnel: 1 - Math.abs(nrm.z / len),
				front,
				depth:
					face.reduce((s, i) => {
...
/**
 * Turn a frame on the spot until whatever stands in it is looking at you.
 *
