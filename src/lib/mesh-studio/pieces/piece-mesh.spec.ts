 ...
 *
 *  Through `fround` FIRST, and that is not decoration. One side of the
 *  comparison below is an f64 source vertex and the other has been through an
 *  f32 buffer, so the grid was being asked to absorb ~1e-7 of difference — which
 *  it does everywhere except within 1e-7 of a boundary, where `Math.round` is
 *  discontinuous and the two sides land on different integers. That made the
 *  test a coin-flip on the low bits of any new vertex: `gauge`'s needle tip
 *  came out at 1.7326490 and split. Rounding both sides through the same f32
 *  image leaves the grid nothing to absorb. Idempotent for the emitted side. */
const grid = (x: number) => Math.round(Math.fround(x) * 1e6);
...
describe('the settlement, flattened', () => {
	...
	for (const [id, piece] of entries) {
		...
		it(`${id} still holds its ground contract, inside its plot`, () => {
			// The catalogue's own contract, re-asserted on what the GPU will actually
			// receive: the flattening is where a stray offset would creep in. Which
			// contract applies is the piece's own — see `SUSPENDED_PIECES`. A
			// projection must arrive at the GPU still clear of the ground, because
			// losing the gap in the flattening is exactly the silent regression this
			// re-assertion exists to catch.
			let minH = Infinity;
			let maxH = -Infinity;
			let maxE = 0;
			let maxN = 0;
			...
			if (SUSPENDED_PIECES.has(id)) {
				expect(minH, 'suspended, but resting on the ground').toBeGreaterThan(0.2);
			} else {
