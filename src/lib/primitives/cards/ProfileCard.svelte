...
<div
	class="pc-card relative rounded-[3px] border p-4 transition-[border-color,box-shadow,transform] duration-[250ms] sm:p-6
	...
	>
		...
		{#if tags.length}
			...
			>
				{#each tags as tag}
					<span
						class="rounded-[2px] border border-[var(--border)] bg-[rgba(255,255,255,0.03)] px-2 py-[0.2rem] font-[var(--mono)] text-[0.5rem] uppercase tracking-[0.15em] text-[var(--fg-dim)]"
					>
						{tag}
					</span>
				{/each}
