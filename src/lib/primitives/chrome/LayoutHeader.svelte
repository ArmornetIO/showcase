...
<style>
	...
	.header-inner {
		...
		/* Falls through to the shell's `--page-gutter` / `--page-content-max`
		   (defined once, on PagePanel) before its own default. That fall-through
		   is what keeps the header's content on the same column as the body it
		   introduces — the header's own vars stay as a per-page override. */
		max-width: var(--header-content-max, var(--page-content-max, none));
		margin-inline: auto;
		padding-inline: var(--header-gutter, var(--page-gutter, 1.5rem));
	}
