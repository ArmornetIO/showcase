<script lang="ts">
	...
	function place() {
		...
		// Compute into locals and publish once. Clamping in place (`x = clamp(x)`)
		// makes this a read-then-write of the same state inside the reposition
		// effect, which re-runs the effect on every placement — one jittering
		// measurement then loops until Svelte throws effect_update_depth_exceeded.
		let nx: number;
		let ny: number;
		if (placement === 'top' || placement === 'bottom') {
			nx = r.left + r.width / 2;
			ny = placement === 'top' ? r.top - GAP : r.bottom + GAP;
			...
			if (half) nx = Math.min(Math.max(nx, half + MARGIN), vw - half - MARGIN);
		} else {
			nx = placement === 'left' ? r.left - GAP : r.right + GAP;
			ny = r.top + r.height / 2;
			const half = (tipEl?.offsetHeight ?? 0) / 2;
			if (half) ny = Math.min(Math.max(ny, half + MARGIN), vh - half - MARGIN);
		}
		x = nx;
		y = ny;
	}

	// The panel is `position: fixed`, but a transformed/filtered ancestor turns it
	// into a containing block: the panel then lays out inside that ancestor and
	// counts toward its scroll extent, so showing a tooltip near the end of a
	// scrollable row nudges the scroll position, which fires the reposition
	// listener, which moves the panel again. Parking it on <body> keeps viewport
