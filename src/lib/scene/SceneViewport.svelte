<script lang="ts">
	...
	/**
	 * Screen → canvas for the drop point: the inverse of `world*tk + t`, with the
	 * element rect subtracted first.
	 *
	 * This duplicates `Canvas`'s own `screenToCanvas`, and not because that
	 * helper is wrong — it subtracts the root rect correctly. It duplicates it
	 * because the helper is published on the canvas CONTEXT, and this component
	 * is Canvas's PARENT: context flows down, so there is no way to reach it
	 * from here.
	 *
	 * Safe to keep in step because `.vp-in` is `inset: 0` and `.cv-root` fills
	 * it, so the two elements share a rect exactly. If that nesting ever gains
	 * padding or a border, this drifts and drops land off by that much.
	 */
	...
	/**
	 * Backdrop strips, in the order they were authored.
	 *
	 * Their placement comes from the object's `place`, not from the strip's own
	 * left/top: once a strip is a scene object, the scene owns where it is, and
	 * two sources of position would immediately disagree the first time anyone
	 * dragged one.
	 */
	const stripObjects = $derived(
		objects
			.filter((o) => o.kind === 'backdrop.strip' && o.strip)
			.map((o) => {
				const p = placement.get(o.id);
				return { ...o.strip!, id: o.id, left: p?.x ?? o.strip!.left, top: p?.y ?? o.strip!.top };
			})
	);
	...
	>
		<Canvas bind:camera minZoom={0.2} maxZoom={3}>
			{#if isGlobe && globePose}
				...
				/>
			{/if}
			<!-- Scenery first, so it paints behind the mesh it sits behind. -->
			{#if stripObjects.length}
				<HorizonBackdrop strips={stripObjects} />
			{/if}
			<MeshStudio
				concept="instrument"
