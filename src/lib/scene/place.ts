...
export function resolvePlacement(scene: Scene): Map<string, Placed> {
	...
	//
	// Backdrop strips join NEITHER. They are scenery: nothing should be pushed
	// aside to make room for a background, and the background should not be
	// shoved about to make room for agents. They fall through to the authored
	// position below, like any object the solve does not touch.
	const nodes = scene.objects.filter((o) => o.kind === 'mesh.node');
