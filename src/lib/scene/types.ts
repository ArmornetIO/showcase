...
export type ObjectKind = 'mesh.node' | 'mesh.edge' | 'component' | 'backdrop.strip';
...
export interface SceneObject {
	...
	/**
	 * A Möbius backdrop strip.
	 *
	 * Authored art rather than solver output, so it is always `pinned` — the
	 * mesh layout has no opinion about where a piece of scenery goes, and
	 * letting it enter the solve would have the separation pass shove the
	 * background around to make room for agents.
	 *
	 * It is a scene object, not a decoration outside the scene, so that beats
	 * and cues can drive it: fading a strip in, turning one, or flying the
	 * camera past it is then the same kind of edit as any other.
	 */
	strip?: StripSpec;
}
...
export interface Scene {
	...
	label: string;
	runMs: number;
	/** Every arrangement, globe included. The globe is not solved to x/y — it
	 *  solves for directions on a sphere and projects them at every moment,
	 *  which `placementAt` handles. Möbius IS solved to x/y: its projection is
	 *  fixed rather than spinning, so it is planar as far as placement cares. */
	layout: MeshLayoutId;
