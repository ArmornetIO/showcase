<script lang="ts">
	...
	const NAV_SECTIONS: NavSection[] = [
		...
		},
		{
			...
			title: 'Examples',
			items: [{ label: 'BREACH', href: `${base}/examples/breach`, icon: 'flag' }]
		},
		{
			title: 'Atoms',
			items: [
	...
	// Mockups are gitignored local sketches, so the set differs per machine and a
	// hardcoded list would be dead links on a fresh clone. `mockupSections` reads
	// what is actually on disk.
	const navSections = $derived([...NAV_SECTIONS, ...mockupSections(base)]);
</script>
...
{:else}
	...
	<AppShell sidebar={sidebarContent as any} mobileBrand={mobileBrand as any} toolbar={showcaseToolbar as any}>
		{@render children()}
	</AppShell>
	<ShowcaseSidebar />
{/if}

<!-- The scene builder's timeline runs the full width of the window, and these are
     position:fixed bottom-right — they sat on top of the last second of every
     scene, making any cue authored there unclickable. -->
{#if !isSceneBuilder && !isScenePlayer}
	<DevCog
		snap={devSnap}
		mode={devMode}
