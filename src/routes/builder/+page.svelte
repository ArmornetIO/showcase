<script lang="ts">
	...
	import { builder, FRAME_PRESETS } from '$lib/builder/store.svelte.js';
	import type { FramePreset } from '$lib/builder/store.svelte.js';
	import LayerControls from '$lib/builder/LayerControls.svelte';
	import ComponentPalette from '$lib/builder/ComponentPalette.svelte';
	...
	import BackdropStudio from '$lib/backdrop/BackdropStudio.svelte';
	import CharacterStudio from '$lib/character/CharacterStudio.svelte';
	// The route is the seam. `src/lib` may not import an example — a library that
	// reaches into one has been turned inside out — but the page that mounts both
	// may, so the studio's POV slot is filled here and nowhere else.
	import { PovPreview } from '$examples/breach/index.js';
	import type { BackdropApply } from '$lib/backdrop/BackdropStudio.svelte';
	...
	let backdropStudioOpen = $state(false);
	let characterStudioOpen = $state(false);
	...
	// ── Palette → canvas ────────────────────────────────────────────────────────
	// Dragging carries `component-id` and the compositor drops it under the
	// cursor. Double-click has no cursor to aim at, so it lands in the middle of
	// whatever the camera is currently looking at.
	let canvasWrapEl = $state<HTMLDivElement | undefined>();

	/** Canvas coordinates of the middle of what the camera is looking at. */
	function canvasCentre(): { x: number; y: number } {
		const rect = canvasWrapEl?.getBoundingClientRect();
		const t = camera?.transform;
		if (!rect || !t) return { x: 0, y: 0 };
		return {
			x: (rect.width / 2 - t.tx) / t.scale,
			y: (rect.height / 2 - t.ty) / t.scale
		};
	}

	function addAtCentre(componentId: string) {
		const at = canvasCentre();
		builder.addItem(componentId, at.x, at.y);
	}
	...
	let framePickerOpen = $state(false);

	/** Drop a new artboard where the camera is looking, like a centre-paste. */
	function addFrame(preset: FramePreset) {
		framePickerOpen = false;
		const rect = canvasWrapEl?.getBoundingClientRect();
		const t = camera?.transform;
		const spec = FRAME_PRESETS[preset];
		if (!rect || !t) {
			builder.addFrame(preset, 0, 0);
			return;
		}
		builder.addFrame(
			preset,
			(rect.width / 2 - t.tx) / t.scale - (spec.w || 1200) / 2,
			(rect.height / 2 - t.ty) / t.scale - (spec.h || 800) / 2
		);
	}

	/** Drop a cluster where the camera is looking, the same way a frame lands.
	 *  Sized to hold a few components rather than a screen — a cluster groups
	 *  part of a composition, not all of one. */
	function addCluster() {
		const rect = canvasWrapEl?.getBoundingClientRect();
		const t = camera?.transform;
		const w = 480;
		const h = 320;
		if (!rect || !t) {
			builder.addCluster(0, 0, w, h);
			return;
		}
		builder.addCluster(
			(rect.width / 2 - t.tx) / t.scale - w / 2,
			(rect.height / 2 - t.ty) / t.scale - h / 2,
			w,
			h
		);
	}
	...
	// ── Panel collapse ───────────────────────────────────────────────────────────
	// Both side panels fold flat to a bare edge, the same gesture AppShell's rail
	// uses: a carat on the panel's inner border, chevron pointing the way it will
	// go. They collapse to nothing rather than to an icon rail — neither panel has
	// an icon vocabulary to fall back to, and the point is canvas width.
	//
	// Persisted for the same reason AppShell persists its rail: on a laptop you
	// collapse them once and mean it, and re-expanding on every reload is the
	// tool overruling you.
	const TOOLBOX_KEY = 'builder:toolbox-collapsed';
	const PROPS_KEY = 'builder:props-collapsed';
	let toolboxCollapsed = $state(false);
	let propsCollapsed = $state(false);

	onMount(() => {
		try {
			toolboxCollapsed = localStorage.getItem(TOOLBOX_KEY) === '1';
			propsCollapsed = localStorage.getItem(PROPS_KEY) === '1';
		} catch {
			/* private mode — the panels just start open */
		}
	});

	function togglePanel(which: 'toolbox' | 'props') {
		const next = which === 'toolbox' ? !toolboxCollapsed : !propsCollapsed;
		if (which === 'toolbox') toolboxCollapsed = next;
		else propsCollapsed = next;
		try {
			localStorage.setItem(which === 'toolbox' ? TOOLBOX_KEY : PROPS_KEY, next ? '1' : '0');
		} catch {
	...
	/**
	 * Chrome that floats OVER the canvas, so a fit has to keep clear of it.
	 *
	 * The side panels are not in here: they are flex siblings, so the canvas
	 * element already ends where they begin and the camera never sees their
	 * width. What does overlap is the tool switch along the top, the page tabs
	 * along the bottom, and the camera controls in the bottom-right corner.
	 */
	const CANVAS_INSETS = { top: 52, bottom: 56, right: 64 };

	/** The tool switch. Shortcuts named in the labels, since a toolbar is where
	 *  people look for them and a tooltip is the only place to put them. */
	const toolItems: IconToolbarItem[] = $derived([
		{
			icon: 'mouse-pointer',
			label: 'Select (V) — drag to move, drag the canvas to marquee',
			active: builder.tool === 'select',
			onclick: () => builder.setTool('select')
		},
		{
			icon: 'hand',
			label: 'Pan (H) — drag anywhere to move the canvas',
			active: builder.tool === 'pan',
			onclick: () => builder.setTool('pan')
		},
		{ divider: true },
		// One fit button, not two. `home` and a `maximize` "fit all" sat here
		// together for a while and were indistinguishable in the common case:
		// with nothing selected they do exactly the same thing. Fit-all still has
		// ⇧⌘H and the header's zoom readout, which is labelled for it.
		{
			icon: 'home',
			label: 'Home (⇧⌘F) — frame the selection, or everything when nothing is selected',
			onclick: recenter
		}
	]);

	// ── Camera readouts ──────────────────────────────────────────────────────────
	// `transform` is the canvas's own `$state`, so reading `.scale` here tracks
	// every pan/zoom without the camera having to notify anyone.
	const zoomPct = $derived(Math.round((camera?.transform.scale ?? 1) * 100));

	/**
	 * The box the camera should frame right now.
	 *
	 * A cluster or a frame answers as its own rect rather than as its contents:
	 * both have padding that is part of the composition, and fitting the items
	 * inside would crop the very edge you drew. Falls back to the selected items,
	 * whose `w`/`h` of 0 means "size yourself" — hence the same minimums the group
	 * bounding boxes use. `null` means nothing is selected.
	 */
	function selectionBox(): { minX: number; minY: number; maxX: number; maxY: number } | null {
		const c = builder.selectedCluster;
		if (c) return { minX: c.x, minY: c.y, maxX: c.x + c.w, maxY: c.y + c.h };
		const f = builder.selectedFrame;
		if (f) return { minX: f.x, minY: f.y, maxX: f.x + f.w, maxY: f.y + f.h };
		const sel = builder.selectionItems();
		if (!sel.length) return null;
		return {
			minX: Math.min(...sel.map((i) => i.x)),
			minY: Math.min(...sel.map((i) => i.y)),
			maxX: Math.max(...sel.map((i) => i.x + Math.max(i.w, 60))),
			maxY: Math.max(...sel.map((i) => i.y + Math.max(i.h, 40)))
		};
	}

	/** Every fit goes through here, so the floating chrome cannot be honoured by
	 *  one caller and forgotten by the next. */
	function fitBox(box: { minX: number; minY: number; maxX: number; maxY: number }) {
		camera?.fitRect(box, { insets: CANVAS_INSETS, duration: 240 });
	}

	function fitEverything() {
		camera?.fitAll({ insets: CANVAS_INSETS, duration: 240 });
	}

	/**
	 * Home. Frames what you are working on — the selected cluster, frame or
	 * items — and the whole canvas when nothing is selected.
	 *
	 * It is a button rather than something automatic because the two moments that
	 * want it are ones only you know about: collapsing a panel hands back width
	 * the camera has no reason to notice, and picking a cluster is a statement of
	 * where you are looking that the tool should not act on uninvited.
	 */
	function recenter() {
	...
	function handleKeyDown(e: KeyboardEvent) {
		...
		if (e.key === 'Delete' || e.key === 'Backspace') {
			// Whatever kind of thing is selected — items, a frame, a connector.
			if (builder.selectionItems().length) {
				e.preventDefault();
				builder.deleteItems(builder.selectionItems().map((i) => i.id));
			} else if (builder.selectedConnectorId) {
				e.preventDefault();
				builder.deleteConnector(builder.selectedConnectorId);
			} else if (builder.selectedFrameId) {
				e.preventDefault();
				// Contents survive: a frame is a region, and deleting the region
				// should not take a composition down with it.
				builder.deleteFrame(builder.selectedFrameId);
			} else if (builder.selectedClusterId) {
				e.preventDefault();
				// Same rule as a frame, and it matters more here: a cluster is what
				// a template lands in, so Delete taking the template with it would
				// be a very expensive misfire.
				builder.deleteCluster(builder.selectedClusterId);
			}
		}

		// V / H — the tool switch every canvas app shares. Unmodified, so they
		// cannot collide with a shortcut, and skipped while typing by the guard
		// at the top of this handler.
		if (!e.metaKey && !e.ctrlKey && !e.altKey) {
			if (e.key === 'v' || e.key === 'V') builder.setTool('select');
			if (e.key === 'h' || e.key === 'H') builder.setTool('pan');
		}
		if (e.key === 'Escape') {
		...
		// Cmd+Z / Shift+Cmd+Z → undo / redo. Checked before everything else so a
		// mis-fire elsewhere is always reversible.
		...
		// Cmd+D duplicate, Cmd+C / Cmd+V copy-paste — all three act on whatever
		// `selectionItems()` resolves to, so they behave the same for an item, a
		// multi-selection and a group.
		...
		// Cmd+Shift+H fit all · Cmd+Shift+F recenter on the selection. Both go
		// through the inset-aware fits, or they park content under the tool
		// switch and the page tabs that float over the canvas.
		...
		// Arrow keys nudge: 1px for precision, one grid unit with Shift.
		if (e.key.startsWith('Arrow')) {
			const step = e.shiftKey ? builder.gridSize : 1;
			const dx = e.key === 'ArrowLeft' ? -step : e.key === 'ArrowRight' ? step : 0;
			const dy = e.key === 'ArrowUp' ? -step : e.key === 'ArrowDown' ? step : 0;
			if ((dx || dy) && builder.selectionItems().length) {
				e.preventDefault();
				builder.nudgeSelection(dx, dy);
				return;
			}
		}
	...
	const sortedLayers = $derived.by(() => {
		...
		// Ranked with the palette's scorer rather than a substring pass: layer
		// names are user-typed, so "hdr" should still find "Header row". A group
		// matches on its own name or on any child's, and z-order stands as the
		// ordering until there is a query to rank by.
		return fuzzyRank(layerSearch, entries, (e) =>
			e.type === 'group'
				? [
						[e.group.name, 0],
						...e.children.map((c) => [displayName(c), 80] as [string, number])
					]
				: [[displayName(e.item), 0]]
		);
	});
	...
	function commitRename() {
		...
		if (trimmed) {
			// One inline-rename input serves three kinds of thing, so the id has to
			// say which it is. Pages first: a page id can never collide with a group
			// or item id, and checking it first keeps the older branches untouched.
			if (builder.pages.some((p) => p.id === renamingId)) {
				builder.renamePage(renamingId!, trimmed);
			} else if (builder.groups.some((g) => g.id === renamingId)) {
				builder.renameGroup(renamingId!, trimmed);
			} else {
				builder.renameItem(renamingId!, trimmed);
			}
		}
		renamingId = null;
	}
	...
	function serializeBrief(): string {
		...
		if (sorted.length === 0 && !builder.frames.length)
			return '(empty canvas — drag components from the toolbox to sketch the layout)';

		const sections: string[] = [];

		// Frames come first: they are the screens the components sit on, and
		// reading a component list without knowing which viewport it belongs to
		// is how a mockup ends up laid out for the wrong width.
		if (builder.frames.length) {
			sections.push(
				'SCREENS\n' +
					builder.frames
						.map((f) => {
							const members = builder.frameMembers(f.id);
							const size = `${Math.round(f.w)}×${Math.round(f.h)}`;
							return `- ${f.name} (${size}${f.clip ? ', clipped' : ''}) @ (${f.x}, ${f.y}) — ${members.length} component${members.length === 1 ? '' : 's'}`;
						})
						.join('\n')
			);
		}
				...
				'COMPONENTS\n' +
					sorted
						.map((item, i) => {
							...
							const frame = builder.frames.find((f) => builder.frameMembers(f.id).includes(item));
							const where = frame ? `  [in ${frame.name}]` : '';
							const pos = `@ (${item.x}, ${item.y})  ${wStr} × ${hStr}${where}`;
		...
		// Connectors describe intent, not layout — they are the part of the sketch
		// that says what leads to what, which is exactly what a static screenshot
		// of the canvas would lose.
		if (builder.connectors.length) {
			const nameOf = (end: string | { x: number; y: number }) => {
				if (typeof end !== 'string') return `(${Math.round(end.x)}, ${Math.round(end.y)})`;
			...
			sections.push(
				'FLOW\n' +
					builder.connectors
						.map((c) => `- ${nameOf(c.fromId)} → ${nameOf(c.toId)}${c.label ? ` (${c.label})` : ''}`)
						.join('\n')
			);
		}
...
{#if contextMenu}
	<CanvasContextMenu
		...
		onrename={(id) => {
			...
			if (item) scheduleRename(id, displayName(item));
		}}
	/>
{/if}
...
<!--
	Apply drops a Backdrop onto the canvas carrying everything the studio authored.
	`strips` and `layers` are JSON strings rather than arrays and records because
	`PropDef` has no kind for either — the renderer parses them, and the values
	round-trip through the builder's own save format unchanged.

	`preset` is written too. It used to be omitted, so applying anything other
	than a Möbius composition placed a Backdrop still set to its default `ash-drift`
	and threw the actual selection away.
-->
<!-- No `onapply` yet: the studio cycles the four figures and nothing more, so
     there is nothing for it to hand back to the canvas. -->
<CharacterStudio open={characterStudioOpen} onclose={() => (characterStudioOpen = false)}>
	{#snippet pov(skin, suit)}
		<PovPreview {skin} {suit} />
	{/snippet}
</CharacterStudio>

<BackdropStudio
	open={backdropStudioOpen}
	onclose={() => (backdropStudioOpen = false)}
	onapply={(out: BackdropApply) => {
		// `addItem` places at coordinates and seeds every prop from its default,
		// so the composition is written straight after — `selectedId` is the new
		// item, because `addItem` selects what it places.
		builder.addItem('HorizonBackdrop', 40, 40);
		const id = builder.selectedId;
		if (!id) return;
		builder.setItemProps(id, {
			preset: out.stack,
			blend: out.blend,
			strips: out.strips.length ? JSON.stringify(out.strips) : '',
			// Omitted entirely when no family was touched, so the prop stays at its
			// default and the inspector does not show it as modified.
			layers: Object.keys(out.familyParams).length
				? JSON.stringify({ styles: out.familyStyles, params: out.familyParams })
				: ''
		});
	}}
/>
...
<div class="builder-root">
	...
	<header class="builder-header">
		...
		<div class="builder-toolbar">
			...
			<button
				...
				title="Fit all (⇧⌘H) — ⇧⌘F frames the selection">◎ {zoomPct}%</button
			>
			<div class="toolbar-menu">
				<button
					class="toolbar-btn"
					class:toolbar-btn--active={framePickerOpen}
					onclick={() => (framePickerOpen = !framePickerOpen)}
					title="Add a frame — a named screen-sized region">▭ FRAME</button
				>
				{#if framePickerOpen}
					<div class="toolbar-pop">
						{#each Object.entries(FRAME_PRESETS) as [key, spec]}
							<button class="toolbar-pop-row" onclick={() => addFrame(key as FramePreset)}>
								<span>{spec.label}</span>
								<span class="toolbar-pop-dim">
									{spec.w ? `${spec.w}×${spec.h}` : '1200×800'}
								</span>
							</button>
						{/each}
					</div>
			...
			<button
				class="toolbar-btn"
				onclick={addCluster}
				title="Add a cluster — a region whose contents snap together and move as one"
				>⧉ CLUSTER</button
			>
			<button
				class="toolbar-btn"
			...
			<button
				...
				>◈ THEME STUDIO</button
			>
			<!-- A dialog, not a route. It was briefly a page in `mockups/`, which had
			     two problems: mockups are gitignored, so the tool was never actually
			     committed; and a separate tab meant the composition you authored had
			     no way back to the canvas. As a modal it sits over the work and can
			     hand its result straight to the selected item. -->
			<button
				class="toolbar-btn"
				class:toolbar-btn--active={backdropStudioOpen}
				onclick={() => (backdropStudioOpen = true)}
				title="Open the Backdrop Studio to author a backdrop composition"
				>◐ BACKDROP STUDIO</button
			>
			<button
				class="toolbar-btn"
				class:toolbar-btn--active={characterStudioOpen}
				onclick={() => (characterStudioOpen = true)}
				title="Open the Character Studio to look through the four figures"
	...
	<div class="builder-body">
		<!-- TOOLBOX -->
		<aside class="builder-toolbox" class:panel-collapsed={toolboxCollapsed}>
			<button
				type="button"
				class="panel-carat panel-carat--right"
				onclick={() => togglePanel('toolbox')}
				aria-expanded={!toolboxCollapsed}
				aria-label={toolboxCollapsed ? 'Show components' : 'Hide components'}
				title={toolboxCollapsed ? 'Show components' : 'Hide components'}
			>
				<svg
					class="panel-chevron"
					class:flipped={toolboxCollapsed}
					width="13"
					height="13"
					viewBox="0 0 24 24"
					fill="none"
			...
			<ComponentPalette
				...
				onTemplate={(t) => {
					const at = canvasCentre();
					// Templates are authored top-left-anchored, so the centre point is
					// shifted back by roughly half a screen — dropping a 1280-wide page
					// centred on the camera would put most of it off to the right.
					const box = builder.applyTemplate(t, Math.max(0, at.x - 400), Math.max(0, at.y - 200));
					// Then put the camera on it. Where it LANDED barely matters once the
					// camera follows, which is the point: a 1440-wide template dropped at
					// the current zoom is mostly off-screen, and hunting for it is the
					// first thing you had to do with every template until now.
					if (box) camera?.fitRect(box, { insets: CANVAS_INSETS, duration: 240 });
				}}
			/>
		</aside>

		<!-- CANVAS -->
		<div class="canvas-wrap" class:canvas-split={splitView} bind:this={canvasWrapEl}>
			<!-- Tool switch, over the canvas rather than in the top chrome: it
			     changes what your pointer does HERE, and a control that far from
			     the surface it governs reads as a page-level setting. -->
			<div class="canvas-tools">
				<IconToolbar orientation="horizontal" items={toolItems} />
			</div>
			<Canvas bind:camera tool={builder.tool}>
				<CompositorLayer
					{connectMode}
					onConnectEnd={() => (connectMode = false)}
					onItemContextMenu={(e) => (contextMenu = { x: e.clientX, y: e.clientY })}
					onItemSelect={(id) => {
						if (id) {
			...
			<!-- ── Pages ───────────────────────────────────────────────────────
			     Along the bottom of the canvas, where a spreadsheet or a design
			     tool puts them — a tab strip rather than a dropdown, because the
			     point of pages is moving between them and a menu hides how many
			...
			<div class="page-tabs">
				{#each builder.pages as page (page.id)}
					...
					<div class="page-tab" class:page-tab--active={active}>
						...
						{:else}
							<button
								class="page-tab-btn"
								onclick={() => builder.switchPage(page.id)}
								ondblclick={() => scheduleRename(page.id, page.name)}
								title="{page.name} — double-click to rename"
							>
								{page.name}
								<span class="page-tab-count">{builder.pageItemCount(page.id)}</span>
							</button>
							{#if active && builder.pages.length > 1}
								<button
									class="page-tab-close"
									onclick={() => builder.deletePage(page.id)}
				...
				{/each}
				<button class="page-tab-add" onclick={() => builder.addPage()} title="New page">+</button>
				<button
					class="page-tab-add"
					onclick={() => builder.duplicatePage(builder.activePageId)}
					title="Duplicate this page">⧉</button
				>
			</div>
		</div>
		...
		<aside
			class="builder-props"
			class:builder-props--ai={rightTab === 'ai'}
			class:panel-collapsed={propsCollapsed}
		>
			<button
				type="button"
				class="panel-carat panel-carat--left"
				onclick={() => togglePanel('props')}
				aria-expanded={!propsCollapsed}
				aria-label={propsCollapsed ? 'Show properties' : 'Hide properties'}
				title={propsCollapsed ? 'Show properties' : 'Hide properties'}
			>
				<svg
					class="panel-chevron"
					class:flipped={propsCollapsed}
					width="13"
					height="13"
					viewBox="0 0 24 24"
					fill="none"
			...
			{#if rightTab === 'props'}
				...
				{#if builder.selectionMode === 'none'}
					<div class="layers-panel">
						...
						<!-- Frames and connectors live outside the z-ordered item list —
						     they are the canvas's structure rather than its content — but
						     they still need somewhere to be selected from once nothing is
						     selected, and somewhere to be un-hidden from. -->
						{#if builder.frames.length || builder.connectors.length}
							<div class="layers-aux">
								{#each builder.frames as f (f.id)}
									<div class="layer-row layer-row--aux">
										<button class="layer-name-btn" onclick={() => builder.selectFrame(f.id)}>
											<span class="layer-kind">▭</span>{f.name}
										</button>
										<button
											class="layer-icon-btn"
											title={f.visible ? 'Hide frame' : 'Show frame'}
											onclick={() => builder.toggleFrameVisible(f.id)}>{f.visible ? '◉' : '○'}</button
										>
										<button
											class="layer-icon-btn"
											title="Delete frame"
											onclick={() => builder.deleteFrame(f.id)}>✕</button
										>
									</div>
								{/each}
								{#each builder.connectors as c (c.id)}
									<div class="layer-row layer-row--aux">
										<button class="layer-name-btn" onclick={() => builder.selectConnector(c.id)}>
											<span class="layer-kind">↗</span>{c.label || 'connector'}
										</button>
										<button
											class="layer-icon-btn"
											title="Delete connector"
											onclick={() => builder.deleteConnector(c.id)}>✕</button
										>
									</div>
								{/each}
							</div>
				...
				<!-- ── Frame selected ─────────────────────────────────────── -->
				{:else if builder.selectionMode === 'frame' && builder.selectedFrame}
					{@const frame = builder.selectedFrame}
					<div class="props-header">
						<span class="props-component-name">FRAME</span>
						<button
							class="props-delete-btn"
							onclick={() => builder.deleteFrame(frame.id)}
							title="Delete frame — its contents stay on the canvas">✕</button
						>
					</div>
					<div class="props-list">
						<div class="prop-row">
							...
							<input
								class="prop-input"
								value={frame.name}
								oninput={(e) => builder.updateFrame(frame.id, { name: e.currentTarget.value })}
							/>
						</div>

						<div class="prop-divider">Viewport</div>
						<div class="prop-chips">
							{#each Object.entries(FRAME_PRESETS) as [key, spec]}
								<button
									class="prop-chip"
									class:prop-chip--on={frame.preset === key}
									onclick={() => builder.setFramePreset(frame.id, key as FramePreset)}
									>{spec.label}</button
								>
							{/each}
						</div>
						<div class="prop-row prop-row--info">
							<span class="prop-label">W × H</span>
							<span class="prop-info">{Math.round(frame.w)} × {Math.round(frame.h)}</span>
						</div>
						<div class="prop-row prop-row--info">
							<span class="prop-label">Contains</span>
							<span class="prop-info">{builder.frameMembers(frame.id).length} components</span>
						</div>

						<div class="prop-divider">Behaviour</div>
						<label class="prop-row prop-row--check">
							<input
								type="checkbox"
								checked={frame.clip}
								onchange={(e) => builder.updateFrame(frame.id, { clip: e.currentTarget.checked })}
							/>
							<span class="prop-label">Clip to bounds</span>
						</label>
						<label class="prop-row prop-row--check">
							<input
								type="checkbox"
								checked={frame.locked}
								onchange={() => builder.toggleFrameLocked(frame.id)}
							/>
							<span class="prop-label">Locked</span>
						</label>
						<button class="props-wide-btn" onclick={() => builder.deleteFrame(frame.id, true)}>
							Delete frame + {builder.frameMembers(frame.id).length} components
						</button>
					</div>

				<!-- ── Cluster selected ───────────────────────────────────── -->
				{:else if builder.selectionMode === 'cluster' && builder.selectedCluster}
					{@const cluster = builder.selectedCluster}
					<div class="props-header">
						<span class="props-component-name">CLUSTER</span>
						<button
							class="props-delete-btn"
							onclick={() => builder.deleteCluster(cluster.id)}
							title="Delete cluster — its contents stay on the canvas">✕</button
						>
					</div>
					<div class="props-list">
						<div class="prop-row">
							...
							<input
								class="prop-input"
								value={cluster.name}
								oninput={(e) => builder.updateCluster(cluster.id, { name: e.currentTarget.value })}
							/>
						</div>
						...
						<div class="prop-chips">
							<button
								class="prop-chip"
								class:prop-chip--on={cluster.layout === 'free'}
								title="Members keep their own position and snap to guides"
								onclick={() => builder.updateCluster(cluster.id, { layout: 'free' })}>Free</button
							>
							<button
								class="prop-chip"
								class:prop-chip--on={cluster.layout === 'stack'}
								title="Members flow in order; the cluster resizes to fit"
								onclick={() => builder.updateCluster(cluster.id, { layout: 'stack' })}>Stack</button
							>
						</div>
						{#if cluster.layout === 'stack'}
							<div class="prop-chips">
								<button
									class="prop-chip"
									class:prop-chip--on={cluster.direction === 'vertical'}
									onclick={() => builder.updateCluster(cluster.id, { direction: 'vertical' })}
									>Down</button
								>
								<button
									class="prop-chip"
									class:prop-chip--on={cluster.direction === 'horizontal'}
									onclick={() => builder.updateCluster(cluster.id, { direction: 'horizontal' })}
									>Across</button
								>
							</div>
						{/if}
						<div class="prop-row">
							...
							<input
								...
								max="200"
								step="4"
								value={cluster.padding}
								oninput={(e) =>
									builder.updateCluster(cluster.id, { padding: Number(e.currentTarget.value) })}
							/>
						</div>
						<div class="prop-row">
							...
							<input
								...
								value={cluster.gap}
								oninput={(e) =>
									builder.updateCluster(cluster.id, { gap: Number(e.currentTarget.value) })}
							/>
						</div>
						<div class="prop-row prop-row--info">
							<span class="prop-label">W × H</span>
							<span class="prop-info">{Math.round(cluster.w)} × {Math.round(cluster.h)}</span>
						</div>
						<div class="prop-row prop-row--info">
							<span class="prop-label">Contains</span>
							<span class="prop-info">{builder.clusterMembers(cluster.id).length} components</span>
						</div>
						...
						<label class="prop-row prop-row--check">
							<input
								type="checkbox"
								checked={cluster.locked}
								onchange={() => builder.toggleClusterLocked(cluster.id)}
							/>
							<span class="prop-label">Locked</span>
						</label>
						{#if cluster.layout === 'stack'}
							<button class="props-wide-btn" onclick={() => builder.reflowCluster(cluster.id)}>
								Re-run layout
							</button>
						{/if}
						<button class="props-wide-btn" onclick={() => builder.deleteCluster(cluster.id, true)}>
							Delete cluster + {builder.clusterMembers(cluster.id).length} components
						</button>
					</div>

				<!-- ── Connector selected ─────────────────────────────────── -->
				{:else if builder.selectionMode === 'connector' && builder.selectedConnector}
					{@const conn = builder.selectedConnector}
					<div class="props-header">
						<span class="props-component-name">CONNECTOR</span>
						<button
							class="props-delete-btn"
							onclick={() => builder.deleteConnector(conn.id)}
							title="Delete connector (Delete key)">✕</button
						>
					</div>
					<div class="props-list">
						<div class="prop-row">
							...
							<input
								...
								placeholder="none"
								oninput={(e) =>
									builder.updateConnector(conn.id, { label: e.currentTarget.value || undefined })}
							/>
						</div>
						...
						<div class="prop-chips">
							{#each ['solid', 'dashed', 'dotted'] as style}
								<button
									class="prop-chip"
									class:prop-chip--on={conn.style === style}
									onclick={() =>
										builder.updateConnector(conn.id, {
						...
						<div class="prop-chips">
							{#each ['none', 'arrow', 'open'] as head}
								<button
									class="prop-chip"
									class:prop-chip--on={conn.arrowHead === head}
									onclick={() =>
										builder.updateConnector(conn.id, {
											arrowHead: head as 'none' | 'arrow' | 'open'
										})}>{head}</button
								>
							{/each}
						</div>
						<div class="prop-chips">
							{#each ['var(--accent)', 'var(--fg-muted)', '#f87171', '#fbbf24', '#a5b4fc'] as color}
								<button
									class="prop-swatch"
									class:prop-swatch--on={conn.color === color}
									style:background={color}
									aria-label={color}
									onclick={() => builder.updateConnector(conn.id, { color })}
								></button>
							{/each}
						</div>
					</div>
...
<style>
	...
	.page-tabs {
		...
		align-items: center;
		gap: 2px;
		max-width: calc(100% - 90px); /* clear of the camera controls */
		padding: 3px;
	...
	.builder-title {
		...
		color: var(--accent);
		/* The page tabs take the slack now, so the strip grows leftward from the
		   toolbar instead of the title pushing everything right. */
		flex: none;
	}
	...
	.toolbar-btn {
		/* Explicit inline-flex and text-decoration rather than button defaults, so
		   the class also fits an <a> if a toolbar entry ever needs to be a link. */
		display: inline-flex;
		align-items: center;
		text-decoration: none;
		white-space: nowrap;
	...
	.toolbar-btn:disabled {
		opacity: 0.3;
		cursor: default;
		border-color: var(--border, rgba(94, 234, 212, 0.15));
		color: var(--fg-dim);
	}
	.toolbar-btn--icon {
		min-width: 30px;
	...
	.builder-toolbox {
		...
		background: var(--bg-elev, #0a1120);
		/* `visible`, not `hidden`, so the edge-carat can hang past the border —
		   the same reason AppShell's sidebar does it. Safe because the palette
		   inside owns its own scrolling (`overflow-y: auto`), so nothing here
		   relied on the aside to contain it. */
		overflow: visible;
		position: relative;
	...
	/* ── Panel collapse ───────────────────────────────────────────────────────
	   Both side panels fold to a bare 1px edge, leaving only the carat. Width is
	   what the canvas wants back, so the panel gives up all of it — the contents
	   are hidden rather than reflowed, because a 0-width flex column would
	   otherwise squash its children instead of clipping them. */
	.builder-toolbox.panel-collapsed,
	.builder-props.panel-collapsed {
		width: 0;
		padding-left: 0;
		padding-right: 0;
	}
	.panel-collapsed > :global(*:not(.panel-carat)) {
		display: none;
	}

	/* Same edge-carat as AppShell's rail toggle: a chevron on the panel's inner
	   border, pointing the way the panel will go. */
	.panel-carat {
		position: absolute;
		top: 50%;
	...
	/* Top-centre, mirroring CameraControls in the opposite corner. Above the
	   canvas's own z-stack; the wrapper is inert so it never eats a drag that
	   was aimed at the surface behind it. */
	.canvas-tools {
		position: absolute;
		top: 10px;
		left: 50%;
	...
	.canvas-tools :global(.icon-toolbar) {
		pointer-events: auto;
	}
	...
	.builder-props {
		...
		border-left: 1px solid var(--border, rgba(94, 234, 212, 0.12));
		background: var(--bg-elev, #0a1120);
		/* See .builder-toolbox — the carat hangs past the left border. */
		overflow: visible;
		position: relative;
		transition: width 0.18s cubic-bezier(0.4, 0, 0.2, 1);
	}
	...
	/* Frames + connectors: structure, listed above the z-ordered item list. */
	.layers-aux {
		border-bottom: 1px solid var(--border, rgba(94, 234, 212, 0.1));
		padding: 4px 0;
	}
