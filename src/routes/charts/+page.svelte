<script lang="ts">
	...
	// ── Gauge ─────────────────────────────────────────────────────────────────
	// A value against a scale somebody else defined. The reading is the series
	// point, the scale is `yAxis.domain`, and every marked value — a gate, a
	// peer median, an SLA — is a `line` annotation on the y-axis.
	const gaugeConfig: ChartConfig = {
		type: 'gauge',
		series: [{ id: 'score', label: 'Vendor score', points: [{ x: 0, y: 76 }] }],
		yAxis: { domain: [0, 100] },
		gauge: {
			label: 'pass ≥ 85',
			bands: [
				{ to: 60, color: '#FB7185', label: 'fail' },
				{ to: 85, color: '#FBBF24', label: 'below gate' },
				{ to: 100, color: '#34D399', label: 'clears' }
			]
		},
		annotations: [{ type: 'line', axis: 'y', value: 85, color: 'var(--accent)', label: '85' }],
		legend: { position: 'none' },
		tooltip: { enabled: false },
		padding: { top: 4, right: 4, bottom: 4, left: 4 }
	};

	// The same type with no bands and a half-circle sweep — the plain case, to
	// show that everything above is optional.
	const gaugePlainConfig: ChartConfig = {
		type: 'gauge',
		series: [{ id: 'load', label: 'Queue depth', points: [{ x: 0, y: 34 }] }],
		yAxis: { domain: [0, 50], unit: '' },
		gauge: { label: 'of 50 slots', sweep: 180, startAngle: 180, needle: false },
		legend: { position: 'none' },
		tooltip: { enabled: false },
		padding: { top: 4, right: 4, bottom: 4, left: 4 }
	};
...
<div class="px-3 sm:px-6 py-4 sm:py-5">
	...
	<div class="two-col">
		...
		<ShowcaseBlock component="Chart">
			<h3 class="demo-label">Gauge</h3>
			<p class="demo-desc">
				Bands are declared by upper bound, so the scale cannot be given gaps or overlaps. The
				filled arc is cut at those boundaries — a single hue would cover the very bands it is read
				against. The gate is a y-axis annotation.
			</p>
			<div style="height: 200px">
		...
		<ShowcaseBlock component="Chart">
			<h3 class="demo-label">Gauge — bare, 180° sweep</h3>
			<p class="demo-desc">No bands, no needle, custom sweep. Everything but the reading is optional.</p>
			<div style="height: 200px">
				<Chart config={gaugePlainConfig} />
			</div>
		</ShowcaseBlock>
