<script lang="ts">
	...
	import LayoutShell from './demos/LayoutShell.svelte';
...
<div class="px-3 sm:px-6 py-4 sm:py-5">
	...
	<PatternSection id="layout" name="Layout" blurb={sections[0].blurb}>
		...
		>
			...
			{#snippet correct()}
				<LayoutShell
					highlight={['org', 'you', 'page']}
					note="Left column anchors, both pinned. Top-right is free to swap per route."
				/>
			{/snippet}
			...
			{#snippet incorrect()}
				<LayoutShell
					tone="incorrect"
					highlight={['nav', 'you', 'page']}
					person="in-list"
					actions="slab"
					note="Person-scope items in the scrolling list, and the unseen count scrolls away with them."
				/>
			{/snippet}
		</PatternSpec>
		...
		>
			...
			{#snippet correct()}
				<LayoutShell
					highlight={['eyebrow', 'hero']}
					note="Click the eyebrow. The row holds still — the explanation is one click away, not gone."
				/>
			{/snippet}
			...
			{#snippet incorrect()}
				<LayoutShell
					...
					highlight={['hero']}
					hero="permanent"
					note="No toggle, so the hero is paid for on visit four hundred and the register starts at the fold."
				/>
			{/snippet}
		</PatternSpec>
		...
		>
			...
			{#snippet correct()}
				<LayoutShell
					highlight={['trail', 'eyebrow']}
					trail="inline"
					note="One row, one voice. The trail carries only ancestors — the page you are on is the eyebrow."
				/>
			{/snippet}
			...
			{#snippet incorrect()}
				<LayoutShell
					...
					highlight={['trail']}
					trail="bar"
					note="Two rows saying the same name twice, and the trail is flush left while the header is not."
				/>
			{/snippet}
		</PatternSpec>
	</PatternSection>
