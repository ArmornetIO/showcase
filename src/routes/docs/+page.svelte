...
<p>The Agent Mesh is a peer-to-peer overlay network that lets autonomous agents discover each other and exchange signals without a central broker. Each node maintains a local view of its neighbors and propagates state changes over its <strong>Agent Line</strong>.</p>
...
<div class="px-3 sm:px-6 py-4 sm:py-5">
	...
	<ShowcaseBlock component="DocsShell">
		<h3 class="component-name">DocsShell</h3>
		<p class="component-desc">
			The assembled page rather than a part of it: <code class="demo-code">DocsShell</code> wraps a
			document's body with the TOC drawer, the metadata block and the layout dashboard, and honours
			the <code class="demo-code">docsLayout</code> settings those controls write — metadata above,
			below or hidden. The pieces above are demoed individually; this is what they add up to.
		</p>
		<p class="component-desc sub">
			<code class="demo-code">topOffset</code> is the height of your app's fixed header, so the TOC
			drawer starts below it rather than under it.
		</p>
		<div class="docs-shell-frame">
			<DocsShell meta={demoMetaApproved} topOffset="0px">
				{#snippet breadcrumbs()}
					<DocsBreadcrumbs items={[{ label: 'Docs', href: '/docs' }, { label: 'Agent Mesh' }]} />
				{/snippet}
				<Prose html={demoProseHtml} />
